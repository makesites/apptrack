...
(function (lib) {
	...
	var apptrack, _at, init, headers;

	var defaults = {
		checkin: false, // automatically register  event for unique visitors and views
		key: false,
		host: "{{HOST}}",
		routes: {
			input: "{{INPUT_ROUTE}}"
	...
	// initialized options
	if( typeof window.apptrack == "object" ){
		init = window.apptrack;
	}
	...
	apptrack.log = function( action, data ){
		if( !_at ) return console.log("Apptrack didn't initialize");
		_at.log( action, data );
	};
	...
	// connect session with tracking
	apptrack.session = function( data ){
		// fallbacks
		data = data || {};
		// alias for set
		_at.set({
	...
	var Apptrack = function( options ){
		...
		this.store = pickStore( this.options.persist );
		...
		if( this.options.checkin ) this.checkIn(); // should this method be exposed, to trigger manuall?
	...
	Apptrack.prototype = {
		...
		log: function(action, data){
			...
			var value = {
				...
				// backwards compatible with 'header' (<=0.6.0)
				headers: this.data['headers'] || this.data['header'] || headers || null
			};
			// save the data locally
			this.store.set(key, JSON.stringify( value ) );
		},

		// set session or header data
		set: function( obj ){
			// prerequisites
			...
			for(var type in obj){
				// supported data
				if( type != "session" && type != "headers" ) continue;
		...
		sync: function(){
			...
			var url = this.options.host + this.options.routes.input;
			// automatically include key to url?
			if( this.options.key ) url += "/"+ this.options.key;
			ajax( url, { method: "POST", data: JSON.stringify(data) }, function( response ){
		...
		checkIn: function(){
			// get a session store
			var store = pickStore(false);
			// prerequisite
			if( !store ) return console.log("there is no available store");
			// check if user registered
			var session = store.get('_apptrack_session');
			//
			if( !session ){
				// setup session
				session = {
					action: 'unique_user',
					params: { user: generateUid() }
				};
				// save the data is session store
				store.set('_apptrack_session', JSON.stringify( session ) );
				// log event with remote store
				var key = "apptrack_"+ generateUid();
				this.store.set(key, JSON.stringify( session ) );
			}
			// log a view regardless
			var key = "apptrack_"+ generateUid();
			var value = {
				action: 'page_view',
	...
	function pickStore( persist ){
		...
		if( !persist && typeof sessionStorage != "undefined" && sessionStorage !== null ){
			// choose localStorage
			store = stores.sessionStorage;
		} else if( persist && typeof localStorage != "undefined" && localStorage !== null ){
			// choose localStorage
	...
	function getHeaders(){
		...
		req.onload = function(){
			// get raw headers
			var raw = req.getAllResponseHeaders().toLowerCase();
			// convert to object
			var rows = raw.split("\n");
			for( var i in rows ){
	...
	// initialize if options available
	if( init ) apptrack( init );
